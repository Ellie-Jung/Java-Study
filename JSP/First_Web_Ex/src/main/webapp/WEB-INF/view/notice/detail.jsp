			...
			<main>
				...
				<div class="margin-top first">
						...
						<table class="table">
							<tbody>
								<tr>
									<!-- request 객체에 담은거 꺼낼때 request.getAttribute("title") -->
									<th>제목</th>
								...
								<tr>
									...
									<c:forTokens var="fileName" items="${n.files}" delims="," varStatus="">
									
											<c:set var="style" value=""/>
										<c:if test="${fn:endsWith(fileName,'.zip') }"> <!-- 확장자가 zip인 경우에 스타일이 적용됨 -->
											<c:set var="style" value="font-weight:bold; color:red;"/>
										</c:if>
										<a href="${fileName}" style="${style}">${fn:toUpperCase(fileName)}</a>  <!-- 출력되는 파일명을 대문자로 바꾸기 -->
										<c:if test="${!st.last}">
										/
										</c:if>
									</c:forTokens>
