...
from emp e, emp e2
where e.job=e2.job and e2.empno=7788;
...
--인라인 뷰
select min(avg)
from emp,(select avg(sal) as avg from emp group by job);
...
from emp e
where sal in(select min(sal) from emp m where e.deptno =m.deptno  group by deptno);
